...
`tauri build` output lands at `src-tauri/target/release/bundle/macos/Markdiff.app`. Replace the installed copy and `lsregister` it, or every `open`/Finder test exercises the OLD binary. Also: `strings` cannot verify frontend changes made it into the binary — Tauri brotli-compresses `dist/` — use binary-vs-source timestamps. If Markdiff is running, don't quit it (unsaved edits live only in memory, D-003): `ditto` the bundle to `Markdiff.app.new`, `mv` the old one aside, `mv` the new one in, `lsregister -f`. The running instance keeps the old code until she relaunches — say so.
...
Lighter options, used 2026-09-24:
- **Frontend-only logic** (tree, filter, stash): extract functions from `dist/index.html` by name with a brace-matching grab and `eval` them in node against `fs` stubs. `window.__TAURI__` is absent in the browser pane, so the page can't run there.
- **IPC/fs smoke after a Tauri bump**: add a temporary IIFE at the end of the script that `readTextFile`s a repo file and `writeTextFile`s the result to the scratchpad. Build, run the bundle binary directly as a second instance, read the file, restore from backup, `grep -c` for the probe, rebuild clean. It does NOT cover the dialogs or Finder's Apple Event open.
...
### Tree state is keyed by folder-relative path — never resolve a save or open target through it
`fileEntries`, `activeFilePath` and `modifiedFiles` use paths relative to the open folder, so `README.md` names a different file after a folder switch. Saving through `fileEntries.get(activeFilePath)` wrote folder A's text into B's same-named file (fixed 2026-09-24, D-006). Invariants: save targets `currentFilePath` (absolute); `scanDirectory` re-derives the relative keys and ignores a superseded scan; any `await` that later binds `activeFilePath` must re-check `fileEntries.get(relPath) === entry`. Never globally re-sort `fileTree`: `walkDir`'s pre-order output IS the display order.
